...
namespace Element
{
    public class DialogService
    {
        ...
        public async Task<DialogResult> ShowDialogAsync<TComponent>(string title)
            where TComponent : ComponentBase
        {
            return await ShowDialogAsync<TComponent>(title, new Dictionary<string, object>(), 0);
        }
        /// <summary>
        /// 显示一个窗口
        /// </summary>
        ...
        public async Task<DialogResult> ShowDialogAsync<TComponent>(string title, IDictionary<string, object> parameters)
            where TComponent : ComponentBase
        {
            return await ShowDialogAsync<TComponent>(title, parameters, 0);
        }
        ...
        /// <param name="parameters">显示该组件所需要的参数</param>
        /// <returns></returns>
        public async Task<DialogResult> ShowDialogAsync<TComponent>(string title, IDictionary<string, object> parameters, float width = 0)
            where TComponent : ComponentBase
        {
            var taskCompletionSource = new TaskCompletionSource<DialogResult>();
            var option = new DialogOption()
